...
const DefaultInput = forwardRef(({ zeroOk, nonZero, valueChange, children, onlyNumber, label, value, containerClassName, onInput, customType, rules, maxLength, ...props }: CustomInputProps, ref) => {
    ...
    return <>
        <div className={`custom-input-wrapper${containerClassName ? (' ' + containerClassName) : ''}${(customType || rules) ? ' has-alert' : ''}`}>
            ...
            {(customType || rules) && <div className="custom-type-alert-text">
                {isAlert ? alertMsg : ''}
            </div>}
        </div>
    </>
