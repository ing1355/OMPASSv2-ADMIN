...
import { DevicesType, GetUsersDetailsApiType, OmpassInfoType, UserType, userRoleType } from 'Types/ServerResponseDataTypes';
...
import { DeletePasscodeFunc } from 'Functions/ApiFunctions';
...
const InformationDetail = () => {
  ...
  const selfIsAdmin = role === 'ADMIN' || role === 'ROOT'
  const isSelf = userId === selectedUuid
  ...
  }, [isModify, rendering, selectedUuid])
  ...
  const createWebAuthnBrowserComponent = (data: DevicesType, b_type: string, img: string) => {
    const { ompassInfo } = data
    const { type, browser } = ompassInfo
    return type === 'WEBAUTHN' && browser === b_type &&
      <div>
        <img src={img} className='information_detail_img_resizing' />
        <ul>
  ...
  return (
    <>
      <Contents>
        <ContentsHeader title={<div>{
          (role !== 'USER' && isAdmin) ? <Link to='/UserManagement'>
            <FormattedMessage id='USER_MANAGEMENT' />
          </Link> : <Link to='/Information'>
            <FormattedMessage id='USER_MANAGEMENT' /> / <FormattedMessage id='USER_LIST' />
          </Link>
        }</div>} subTitle={(role !== 'USER' && isAdmin) ? 'USER_REGISTRATION_INFO' : 'USER_REGISTRATION_INFO'} style={{
          width: '1100px'
        }} />
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            {
              isSelf && role !== 'ROOT' ? modifyFun() :
              (!isAdmin && modifyFun())
            }
            {/* {(role === 'ROOT' || role === 'USER') ?
              modifyFun()
              :
              (role === 'ADMIN' && (adminIdInfo.isAdmin && adminIdInfo.adminId === username || userData?.role === 'USER')) ?
                modifyFun()
                :
                <></>
            } */}
              ...
              onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
                ...
                if (name && password && user_password && password && phoneNumber && !isNameAlert && !isPasswordAlert && !isPasswordConfirmAlert && !isPhoneAlert) {
                  CustomAxiosPut(
                    ...
                    {
                      id: (role === 'USER' ? userId : selectedUuid),
                      name: name,
                      password: password,
          ...
          >
                ...
                {data.ompassInfo === null ?
                  ...
                  <>
                    ...
                    >
                      ...
                      {createWebAuthnBrowserComponent(data, 'Chrome', chrome_img)}
                      {createWebAuthnBrowserComponent(data, 'Chrome Mobile', chrome_mobile_img)}
                      {createWebAuthnBrowserComponent(data, 'Microsoft Edge', microsoft_edge_img)}
                      {createWebAuthnBrowserComponent(data, 'Firefox', firefox_img)}
                      {createWebAuthnBrowserComponent(data, 'Safari', safari_img)}
                      {createWebAuthnBrowserComponent(data, 'Mobile Safari', safari_mobile_img)}
                      {createWebAuthnBrowserComponent(data, 'Samsung Browser Mobile', samsung_browser_mobile_img)}
                    </div>
                  </>
                }
            ...
            >
              ...
              <div className='information_detail_update_date' style={{ width: '55%', marginLeft: '50px' }}>
                <FormattedMessage id='LAST_LOGIN' /> : {data.ompassInfo && data.ompassInfo.updateAt ? data.ompassInfo.updateAt : '-'}
            ...
            >
              ...
              >
                ...
                {role === 'ADMIN' && !data.passcode && ((adminIdInfo.isAdmin && adminIdInfo.adminId === username) || userData?.role === 'USER') &&
                  viewPasscodeSettings[index] &&
                  <div
                    className='button-st4 information_detail_save_btn'
                    onClick={(e) => {
                          ...
                          {role === 'ROOT' &&
                            <>
                              ...
                              <Popconfirm
                                ...
                                onConfirm={() => {
                                  // DeletePasscodeFunc(data.passcode.id, () => {
